...
Meteor.users.helpers({
  ...
  getFirstName() {
    if (this.profile && this.profile.firstName) {
      ...
      this.services.facebook.first_name
    ) {
      return this.services.facebook.first_name;
    } else if (
      this.services &&
      this.services.linkedin &&
      this.services.linkedin.firstName
    ) {
  ...
  getLastName() {
    if (this.profile && this.profile.lastName) {
      ...
      this.services &&
      this.services.google &&
      this.services.google.family_name
    ) {
      return this.services.google.family_name;
    } else if (
      this.services &&
      this.services.facebook &&
      this.services.facebook.last_name
    ) {
      return this.services.facebook.last_name;
    } else if (
      this.services &&
      this.services.linkedin &&
      this.services.linkedin.lastName
    ) {
  ...
  verifiedEmail() {
    return (
      (this.emails && this.emails[0].verified) ||
      this.services.google ||
      this.services.facebook ||
      this.services.linkedin
    );
  ...
  isProfileComplete() {
    if (
      ...
      this.profile.address.countryCode &&
      this.profile.address.country &&
      this.profile.address.province &&
      this.profile.address.postalCode &&
  ...
  hasRoutine(routineId) {
    if (this.routines) {
      for (let r = 0; r < this.routines.length; r++) {
        if (this.routines[r]._id == routineId) {
          return true;
        }
      }
      return false;
